...
const { validateOrgOrPersonalNumber } = require('./isPersonnummerOrOrganisationsnummer.js');
const { valid, isOrg, msg } = validateOrgOrPersonalNumber(number);

const type = isOrg ? 'organisationsnummer' : 'personnummer';
console.log(valid ? success : error, valid ? `Number is valid ${type}` : 'Number is invalid');
if (!valid) {
